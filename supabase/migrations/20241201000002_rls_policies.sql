...
-- Create function to get user's org_id (in public schema for hosted Supabase)
...
CREATE POLICY "Users can view cash envelopes for their venues"
  ...
  USING (venue_id IN (SELECT id FROM venues WHERE org_id = public.get_user_org_id()));

CREATE POLICY "Users can insert cash envelopes for their venues"
  ON cash_envelopes FOR INSERT
  WITH CHECK (venue_id IN (SELECT id FROM venues WHERE org_id = public.get_user_org_id()));
